...
struct RequestUserPayload : public Payload {
  char  name[255];
};

struct RequestFilePayload : public Payload {
  char file_name[255];
};

struct RequestPublicKeyPayload : public RequestUserPayload {
  char public_key[160];
};

struct RequestSentFilePayload : public RequestFilePayload {
  uint32_t content_size;
  char message_content[255];
};

struct ResponseEmptyPayload : public Payload {};

struct ResponsePayload : public ResponseEmptyPayload {
  char client_id[16];
};

struct ResponseKeyPayload : public ResponsePayload {
  char encrypted_key[];
};

struct ResponseReceiveFilePayload : public ResponsePayload {
  uint32_t content_size;
  char file_name[255];
  uint32_t cksum;
}
